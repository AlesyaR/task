""" Данная программа осуществляет поиск в iplir.conf и определяет - содержатся ли в указанной секции id, указанные записи (записей не меньше 1го).
    
    * В случае положительного результата - выводит сообщение о успешном завершении;

    * В случае отрицательного результата - выводит какую из записей в нужном id или сам id, если он не был найден.

    Аргументы содержатся в конфигурационном файле:
